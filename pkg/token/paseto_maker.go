...
func NewPasetoMaker(symmmetricKey string) (Maker, error) {
...
func (maker *PasetoMaker) CreateToken(username string, role string, duration time.Duration) (string, *Payload, error) {
	...
	token, err := maker.paseto.Encrypt(maker.symmmetricKey, payload, nil)
...
func (maker *PasetoMaker) VerifyToken(token string) (*Payload, error) {
	...
	err := maker.paseto.Decrypt(token, maker.symmmetricKey, payload, nil)
